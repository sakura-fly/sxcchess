...
public class Chess {
    public static void main(String args[]) {
        ...
        ChessMainFrame chess = new ChessMainFrame("中国象棋：观棋不语真君子，棋死无悔大丈夫");
        Competitor c = Competitor.getInstance();
        c.setA_Chess(chess);
    }
}
...
class ChessMainFrame extends JFrame implements ActionListener, MouseListener, Runnable {
    ...
    //保存当前操作   Vector向量，相当于链表，装对象，不用指定大小
    Vector Var;
...
//	*/
    ChessMainFrame(String Title) {
        ...
        contact = new JButton("连接计算机");
        contact.setToolTipText("输入IP地址可以连接到另一台计算机");
        ...
        //把组件添加到工具栏
        jmain.setLayout(new GridLayout(0, 5));
        jmain.add(contact);
        jmain.add(anew);
        jmain.add(repent);
        jmain.add(exit);
        ...
        image.addMouseMotionListener(new Frame1_this_mouseMotionAdapter(this));
    ...
    public void run() {
        while (true) {
            ...
            if (chessManClick) {
                if (chessPlayClick == 1) {
                    if (Man == 0 || Man == 1 || Man == 4 || Man == 5 || Man == 8 || Man == 9 || Man == 12 || Man == 13 || Man == 16 || Man == 17 || Man == 18 || Man == 19 || Man == 20 || Man == 26 || Man == 27 || Man == 30)
                    ...
                    if (Man == 2 || Man == 3 || Man == 6 || Man == 7 || Man == 10 || Man == 11 || Man == 14 || Man == 15 || Man == 21 || Man == 22 || Man == 23 || Man == 24 || Man == 25 || Man == 28 || Man == 29 || Man == 31)
                        play[Man].setVisible(false);
...
//		}
        ...
        if (me.getSource().equals(image)) {
            ...
            if (chessPlayClick == 2 && play[Man].getName().charAt(1) == '2') {
                ...
                if (WebSocketClient.session != null) {
                    ...
                    sendSB.append("<Red>").append(',');
                    sendSB.append(String.valueOf(me.getX())).append(',');
                    sendSB.append(String.valueOf(me.getY())).append(',');
                    sendSB.append(String.valueOf(Man)).append(',');
                }
                //移动卒、兵
                if (Man > 15 && Man < 26) {
            ...
            //当前没有操作(停止闪烁)
            ischessManClick = false;
            chessManClick = ischessManClick;
        }//if

        //单击棋子
        else {
            if (WebSocketClient.session != null) {
                sendSB.append("<eat>").append(',');
                sendSB.append(String.valueOf(ischessManClick)).append(',');
            }
            //第一次单击棋子(闪烁棋子)
            if (!ischessManClick) {
                for (int i = 0; i < 32; i++) {
                    ...
                    if (me.getSource().equals(play[i])) {
                        //告诉线程让该棋子闪烁
                        if (WebSocketClient.session != null) {
                            sendSB.append(String.valueOf(i)).append(',');
                        }
                        Man = i;
                        //开始闪烁
                        ischessManClick = true;
                        chessManClick = ischessManClick;
                        break;
                    }
                }//for
                    ...
                    if (me.getSource().equals(play[i])) {
                        ...
                        if (chessPlayClick == 2 && play[Man].getName().charAt(1) == '2') {
                            ...
                            if (WebSocketClient.session != null) {
                                ...
                                sendSB.append(String.valueOf(me.getY())).append(',');
                                sendSB.append(String.valueOf(i)).append(',');
                                sendSB.append(String.valueOf(chessPlayClick)).append(',');
                                sendSB.append(String.valueOf(Man)).append(',');
                            }
                            //卒、兵吃规则
                            if (Man > 15 && Man < 26) {
        ...
        try {
            if (WebSocketClient.session != null) {
                WebSocketClient.session.getBasicRemote().sendText(sendSB.toString());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource().equals(anew)) {
            ...
            if (WebSocketClient.session != null) {
                try {
                    WebSocketClient.session.getBasicRemote().sendText("<Over>");
                    out.close();
                    in.close();
                    socket.close();
                } catch (IOException ex1) {
                }
                JOptionPane.showMessageDialog(this, "网络连接已经断开");
            }
            int i, k;
            //重新排列每个棋子的位置
            //黑色棋子
        ...
        //连接到另一台计算机
        else if (ae.getSource().equals(contact)) {
            // String nick = JOptionPane.showInputDialog("请输入昵称：");
            if (WebSocketClient.session == null) {
                WebSocketClient ws = new WebSocketClient();
                ws.start();
            }
            // try {
            //     WebSocketClient.session.getBasicRemote().sendText("conet," + nick);
            // } catch (IOException e) {
            //     e.printStackTrace();
            // }
            //      boolean isipwrong=false;
            //      do{
            //          int iCount = 0;
            //          int ip[],ipindex[],index=0;
            //          String strIP = "";
            //          ipindex = new int[4];
            //          ip = new int[4];
            //
            //          String ip_address = JOptionPane.showInputDialog("请输入IP地址：");
            //          if(ip_address==null||ip_address.equals("")) return;
            //          strIP = ip_address;
            //
            //         //如果输入的IP地址不是按照“.”分隔成4段，则重新输入
            //         for(int i=0;i<ip_address.length();i++){
            //             String temp = ip_address.substring(i,i+1);
            //             if(temp.compareTo(".")==0) iCount++;
            //         }
            //         if(iCount!=3)
            //             isipwrong=false;
            //         //如果输入的IP地址不符合相应区间，则重新输入
            //         for(int i=0;i<ip_address.length();i++){
            //             String temp = ip_address.substring(i,i+1);
            //             if(temp.compareTo(".")==0) ipindex[index++]=i;
            //         }
            //         ip[0]=Integer.valueOf(ip_address.substring(0,ipindex[0]));
            //         ip[1]=Integer.valueOf(ip_address.substring(ipindex[0]+1,ipindex[1]));
            //         ip[2]=Integer.valueOf(ip_address.substring(ipindex[1]+1,ipindex[2]));
            //         ip[3]=Integer.valueOf(ip_address.substring(ipindex[2]+1,ip_address.length()));
            //
            //         if(ip[0]<1||ip[0]>223)
            //             isipwrong=false;
            //         else if(ip[1]<0||ip[1]>255)
            //             isipwrong=false;
            //         else if(ip[2]<0||ip[2]>255)
            //             isipwrong=false;
            ...
            // 				   WebSocketClient.session.getBasicRemote().sendText("<Connection>");
            ...
            //         }else
            //             JOptionPane.showMessageDialog(this,
            //                     "IP地址输入错误！请重新输入","错误信息",JOptionPane.INFORMATION_MESSAGE);
            // }while(!isipwrong);
    ...
    class ChessRule {
        ...
        public void armsRule(int Man, JLabel play, MouseEvent me) {
            ...
            if (Man < 21) {
                ...
                else if (play.getY() > 284 && (me.getX() - play.getX()) >= 57 && (me.getX() - play.getX()) <= 112 && (me.getY() - play.getY()) > -27 && (me.getY() - play.getY()) < 27) {
                    play.setBounds(play.getX() + 57, play.getY(), 55, 55);
                }

                //向左移动、得到终点的坐标模糊成合法的坐标、必须过河
                else if (play.getY() > 284 && (me.getX() - play.getX()) <= -2 && (me.getX() - play.getX()) >= -58 && (me.getY() - play.getY()) > -27 && (me.getY() - play.getY()) < 27) {
                    //模糊坐标
                    play.setBounds(play.getX() - 57, play.getY(), 55, 55);
                }
            }
            ...
            //红卒向上
                ...
                //向右移动、得到终点的坐标模糊成合法的坐标、必须过河
                else if (play.getY() < 341 && (me.getX() - play.getX()) >= 57 && (me.getX() - play.getX()) <= 112 && (me.getY() - play.getY()) > -27 && (me.getY() - play.getY()) < 27) {
                    play.setBounds(play.getX() + 57, play.getY(), 55, 55);
                }

                //向左移动、得到终点的坐标模糊成合法的坐标、必须过河
                else if (play.getY() < 341 && (me.getX() - play.getX()) <= -2 && (me.getX() - play.getX()) >= -58 && (me.getY() - play.getY()) > -27 && (me.getY() - play.getY()) < 27) {
                    play.setBounds(play.getX() - 57, play.getY(), 55, 55);
                }
            }
        }//卒移动结束
        ...
        public void armsRule(JLabel play1, JLabel play2) {
            //向右走
            if ((play2.getX() - play1.getX()) <= 112 && (play2.getX() - play1.getX()) >= 57 && (play1.getY() - play2.getY()) < 27 && (play1.getY() - play2.getY()) > -27 && play2.isVisible() && play1.getName().charAt(1) != play2.getName().charAt(1)) {
                //黑棋要过河才能右吃棋
            ...
            //向左走
            else if ((play1.getX() - play2.getX()) <= 112 && (play1.getX() - play2.getX()) >= 57 && (play1.getY() - play2.getY()) < 27 && (play1.getY() - play2.getY()) > -27 && play2.isVisible() && play1.getName().charAt(1) != play2.getName().charAt(1)) {
                //黑棋要过河才能左吃棋
                if (play1.getName().charAt(1) == '1' && play1.getY() > 284 && play1.getName().charAt(1) != play2.getName().charAt(1)) {
                    play2.setVisible(false);
                    //把对方的位置给自己
            ...
            //向上走
            else if (play1.getX() - play2.getX() >= -27 && play1.getX() - play2.getX() <= 27 && play1.getY() - play2.getY() >= -112 && play1.getY() - play2.getY() <= 112) {
                //黑棋不能向上吃棋
        ...
        public void willRule(int Man, JLabel play, JLabel playTake, JLabel playQ[]) {
            ...
            //向下
            else if (playTake.getX() - play.getX() >= 0 && playTake.getX() - play.getX() <= 87 && playTake.getY() - play.getY() >= 27 && playTake.getY() - play.getY() <= 140 && playTake.isVisible()) {
                //被吃的棋子是否和当前将相近
                if (Man == 30 && playTake.getX() >= 195 && playTake.getX() <= 309 && playTake.getY() <= 170) {
                    will = true;
                }
...
class Frame1_this_mouseMotionAdapter extends MouseMotionAdapter {
    private ChessMainFrame adaptee;

    Frame1_this_mouseMotionAdapter(ChessMainFrame adaptee) {
